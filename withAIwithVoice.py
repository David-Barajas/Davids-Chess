# File purpose:
# Runs chess app in singleplayer/ai mode with voice enabled inputs
...
class Singleplayer_ChessWV(App):
    # taken from 112 website
    ...
    def appStarted(self):
        ...
        # all piece images taken from the chess wikipedia page :)
    ...
    def takeTurn(self):
        ...
        if (not self.blackCheckmate) and (not self.whiteCheckmate):
            if (self.whiteTurn):
                ...
                bestBoard = None
                bestMove = None
                for board, move in nextBoards:
                    currValue = minimax(board, 1, -1000000000000, 1000000000000, False)
                    if (currValue <= minValue):
                        minValue = currValue
                        bestBoard = board
                        bestMove = move
                audio = gTTS(text=bestMove, lang='en', slow=True) 
                audio.save("move.mp3")
                os.system('mpg123 move.mp3')   
