# File purpose:
# Runs chess app in multiplayer mode with mouse clicking inputs
...
class Multiplayer_ChessNV(App):
    ...
    def drawSideWidget(self, canvas):
        # Basic titles
        canvas.create_rectangle(self.margin, self.margin, self.boardOffset-self.margin,
                                self.height-self.margin, width=6)
        canvas.create_text(self.boardOffset/2, 2*self.margin, anchor='s', 
                        text='2-Player Game', font='Helvetica 30 bold')
        ...
        # displays interpretetation of percentage
        if ('white' in checks):
        ...
        if (self.moveFrom != None) and (self.mouseLocation != None) and\
           (self.mouseLocation in currentPieceMoves):
